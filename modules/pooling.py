...
class AttentivePool(nn.Module):
    ...
    def __init__(self, embed_dim, hidden_dim=64, att_channel=16):
        super(AttentivePool, self).__init__()
        self.fc1 = nn.Linear(embed_dim, hidden_dim)
        self.tanh = nn.Tanh()
        self.fc2 = nn.Linear(hidden_dim, att_channel, bias=False)
    ...
    def forward(self, x):
        ...
        for i in range(1, weights.shape[-1]):
            out = torch.cat((out, (weights[:, :, i].unsqueeze(dim=-1) * x).mean(dim=1)), dim=-1)
        return out
...
class ChannelDepAttentiveStatsPool(nn.Module):
    
    def __init__(self, embed_dim):
        super(ChannelDepAttentiveStatsPool, self).__init__()
    ...
    def forward(self, x):
        # input: batch * frame * embd_dim
        weights = F.softmax(self.fc2(self.tanh(self.fc1(x))), dim=1) # batch * frame * embd_dim
        out = self.stats_pool((weights * x).transpose(1, 2))
        return out

class ASP_pooling(nn.Module):
    ## Attentive statistic pooling
    ## Usage : 
    ## INPUT [B,F,T]
    ## OUPUT [B,embd]
    ##
    # asp = ASP_pooling(128,64)
    # x = torch.randn(10,128,640)
    # asp(x).shape  ##torch.Size([10, 64])
    def __init__(self, in_dim, out_dim) -> None:
        super(ASP_pooling, self).__init__()
