...
from modules.actnorms import ActNorm
import numpy as np
...
class Wide_ResNet(nn.Module):
    def __init__(self, depth, widen_factor=2, num_classes=10, input_channels=3,
                 sum_pool=False, norm=None, leak=.2, dropout_rate=0.0, embedding_size=128, feats_size=60, **kwargs):
        ...
        self.layer1 = self._wide_layer(wide_basic, nStages[1], n, dropout_rate, stride=[1,2])
        self.layer2 = self._wide_layer(wide_basic, nStages[2], n, dropout_rate, stride=[2,4])
        self.layer3 = self._wide_layer(wide_basic, nStages[3], n, dropout_rate, stride=[2,4])
        self.bn1 = get_norm(nStages[3], self.norm)
        self.last_dim = nStages[3]
        # self.linear = nn.Linear(nStages[3], num_classes)
        self.linear2 = nn.Linear(feats_size//2//2*nStages[3], embedding_size)
